(()=>{
    ...
    const getCurrentSection = function()   
    {
        ...
        //부분 높이 설정[첫번째 세션 높이, 첫번째+두번째 세션 높이]
        let segment = [
            sectionSet[0].height,
            sectionSet[0].height + sectionSet[1].height            
        ];

        //y위치가 첫번째 세션높이보다 낮다면 세션 넘버링은 0, 첫 번째 보다
        //높고 두번째 보다 낮다면 세션 넘버링은 1 그이상은 2
        //이후 세션값 리턴
    ...
    const calcValue = function(values)
    {
        ...
        {
            partStart = values[2].start * height;//애니메이션 시작 위치
            partEnd   = values[2].end * height;//애니메이션 종료 위치
            partHeight = partEnd - partStart;//애니메이션 작동 범위

            if (sectionYOffset < partStart)//y값위치가 애니메이션 시작위치보다 작다면 값은 0
            {
                result = values[0];
            
            }
            else if (sectionYOffset > partEnd)//y값위치가 애니메이션 종료위치보다 높다면 값은 1
            {
                result = values[1];
